...
# estas variables son necesarias para poder ejecutar el scrip de forma dinamica
...
calc_indx <- function(col_name){
  ...
  #genera un dataframe con las descripciones de los productos para agregarlos a los
  # indices calculados
...
export_index <- function(){
  ...
  file_name <- paste0("indices/ind_", mes, ".xlsx")
...
readind <- function(months, sheet_names){
  ...
  for (i in seq_along(months)) {
    file_path <- paste0("indices/ind_", months[i], ".xlsx")
